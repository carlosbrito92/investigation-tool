...
# ============================================
# CONSTANTS — TRANSLATIONS & COMPILED REGEX
# ============================================

STATUS_TRANSLATIONS = {
    # Situação Cadastral (Receita Federal / Cadastro Empresa)
    "ativa": "Active", "ativo": "Active",
    "baixada": "Closed", "baixado": "Closed",
    "inapta": "Inactive", "inapto": "Inactive",
    "suspensa": "Suspended", "suspenso": "Suspended",
    "nula": "Null", "nulo": "Null",
    "em liquidação": "In Liquidation", "em liquidacao": "In Liquidation",
    # Whois / Registro.br
    "publicado": "Published", "reservado": "Reserved",
    "expirado": "Expired", "cancelado": "Cancelled",
    "aguardando pagamento": "Awaiting Payment", "on hold": "On Hold",
}

ACTIVITY_HISTORY_FILE = "activities_history.json"
LOCK_FILE = "activities_history.json.lock"

# Compilando RegEx globalmente para alta performance
...
PATTERN_PHONE = re.compile(r'\(\d{2}\)\s*\d{4,5}-\d{4}')
PATTERN_URL = re.compile(r'https?://[^\s\"\'\)\]>]+')

# ============================================
# ACTIVITY HISTORY — PERSISTENCE (Thread-Safe)
# ============================================
...
def load_activity_history():
    if os.path.exists(ACTIVITY_HISTORY_FILE):
        try:
            ...
            logging.error(f"Erro ao carregar o histórico de atividades: {e}")
            return []
    return []
...
def save_activity_to_history(activity: str):
    ...
    try:
        ...
        logging.warning("Timeout ao tentar adquirir o lock do arquivo de histórico.")
    except Exception as e:
        logging.error(f"Falha ao salvar a atividade no histórico: {e}")
...
def extract_cnpj(text):
    match = PATTERN_CNPJ.search(text)
...
def extract_all_emails(text):
    return list(dict.fromkeys(PATTERN_EMAIL.findall(text)))
...
def extract_all_phones(text):
    return list(dict.fromkeys(PATTERN_PHONE.findall(text)))

def extract_urls(text):
    return PATTERN_URL.findall(text)
...
def parse_cadastro_empresa(text: str) -> dict:
    ...
    razao = find([r"Raz[aã]o\s+Social[:\s]+([^\n\r]+)", r"Nome Empresarial[:\s]+([^\n\r]+)", r"Empresa[:\s]+([^\n\r]+)"])
    if razao: result["razao"] = razao

    fantasia = find([r"Nome\s+Fantasia[:\s]+([^\n\r]+)", r"Fantasia[:\s]+([^\n\r]+)"])
    if fantasia and fantasia.lower() not in ["não informado", "nao informado", "-"]:
        result["fantasia"] = fantasia

    foundation = find([r"Data\s+(?:de\s+)?Abertura[:\s]+(\d{2}/\d{2}/\d{4})", r"Abertura[:\s]+(\d{2}/\d{2}/\d{4})", r"Constitui[cç][aã]o[:\s]+(\d{2}/\d{2}/\d{4})", r"Fundada?[:\s]+(\d{2}/\d{2}/\d{4})", r"Data\s+de\s+In[ií]cio[:\s]+(\d{2}/\d{2}/\d{4})"])
    if foundation: result["foundation_date"] = foundation

    city = find([r"Munic[ií]pio[:\s]+([^\n\r\/,]+)", r"Cidade[:\s]+([^\n\r\/,]+)", r"Localidade[:\s]+([^\n\r\/,]+)"])
    if city: result["location"] = city.strip()

    activity = find([r"CNAE/Atividade\s+Principal:\s*\n\s*[\d\.]+[-–]\d+/\d+\s*[-–]\s*([^\n\r]+)", r"Atividade\s+Principal[:\s]+([^\n\r]+)", r"CNAE\s+Principal[:\s]+[\d\.]+[-–][\d/]+\s*[-–]\s*([^\n\r]+)", r"CNAE[:\s]+[\d\.]+[-–][\d/]+\s*[-–]\s*([^\n\r]+)", r"atividade/CNAE\s+principal\s+[\d\.\-/]+\s*[-–]\s*([^,\.]+)"])
    if activity:
    ...
    status = find([r"^Situa[cç][aã]o:\s*(\w+)", r"No momento sua situa[cç][aã]o [eé]\s+(\w+)", r"sua situa[cç][aã]o [eé]\s+(\w+)", r"Status[:\s]+(\w+)"])
    if status: result["current_status"] = translate_status(status)

    address = find([r"Logradouro[:\s]+([^\n\r]+)", r"Endere[cç]o[:\s]+([^\n\r]+)"])
    ...
    if address:
        parts = [address]
        if complemento and complemento.lower() not in ["não informado", "nao informado", "-", ""]: parts.append(complemento)
    ...
    website_match = find([r"Site[:\s]+(https?://[^\s]+)", r"Website[:\s]+(https?://[^\s]+)", r"Homepage[:\s]+(https?://[^\s]+)"])
    if website_match: result["website"] = website_match
...
def parse_whois(text: str) -> dict:
    ...
    if not email: email = extract_email(text)
    if email: result["email"] = email

    phone = find_field(["phone", "registrant phone", "fax-no"])
    if not phone: phone = extract_phone(text)
    if phone: result["telefone"] = phone

    responsible = find_field(["responsible", "admin-c", "tech-c", "registrant name"])
    if responsible: result["partner_name"] = responsible
    ...
    if domain: result["domain"] = domain.lower().strip()
...
def parse_social_media(text: str) -> dict:
    ...
    for url in urls:
        ...
        elif not any(x in url_lower for x in ["facebook", "instagram", "linkedin", "linktree", "bio.site", "beacons", "linktr.ee", "bit.ly"]):
...
def parse_website(text: str) -> dict:
    ...
    for url in urls:
        url_lower = url.lower()
        if "facebook.com" in url_lower: result.setdefault("facebook", url)
        elif "instagram.com" in url_lower: result.setdefault("instagram", url)
        elif "linkedin.com" in url_lower: result.setdefault("linkedin", url)
...
FIELD_LABELS = {
    "domain": "Domain", "cnpj": "CNPJ", "razao": "Legal Name", "fantasia": "Fantasy Name",
    "foundation_date": "Foundation Date", "location": "Location", "main_activity": "Main Activity",
    "current_status": "Current Status", "endereco": "Address", "telefone": "Phone", "email": "Email",
    "partner_name": "Partner Name", "partner_email": "Partner Email", "website": "Website",
    "facebook": "Facebook", "instagram": "Instagram", "linkedin": "LinkedIn",
    "additional_emails": "Additional Emails", "additional_phones": "Additional Phones",
}

def apply_extracted(extracted: dict, overwrite: bool = False) -> tuple[int, list]:
    filled = []
    for key, value in extracted.items():
        if key.startswith("_") or key not in st.session_state.form_data:
            continue
...
def generate_dossier(data: dict) -> str:
    ...
    cnpj = d.get("cnpj", "")
    if is_empty(cnpj): cnpj = extract_cnpj(whois_data) or ""

    partner_name = d.get("partner_name", "")
    if is_empty(partner_name):
        m = re.search(r'owner:\s+([^\n]+)', whois_data, re.I)
        if m: partner_name = m.group(1).strip()
...
with st.sidebar:
    ...
    with tab_cad:
        cad_text = st.text_area("Cole o texto do Cadastro Empresa", height=160, key="cad_raw")
    ...
    with tab_whois:
        whois_text = st.text_area("Cole o output do Whois / Registro.br", height=160, key="whois_raw")
    ...
    with tab_social:
        social_text = st.text_area("Cole o texto da página social", height=160, key="social_raw")
    ...
    with tab_site:
        site_text = st.text_area("Cole o texto do website da empresa", height=160, key="site_raw")
...
with col1:
    st.session_state.form_data['domain'] = st.text_input("🌐 Actionable Domain", value=st.session_state.form_data['domain'])
with col2:
    st.session_state.form_data['cnpj'] = st.text_input("🆔 CNPJ", value=st.session_state.form_data['cnpj'])
with col3:
    st.session_state.form_data['website'] = st.text_input("🔗 Website", value=st.session_state.form_data['website'])

col1, col2 = st.columns(2)
with col1:
    st.session_state.form_data['razao'] = st.text_input("📝 Legal Name (Razão Social)", value=st.session_state.form_data['razao'])
with col2:
    st.session_state.form_data['fantasia'] = st.text_input("🏪 Fantasy Name (Nome Fantasia)", value=st.session_state.form_data['fantasia'])

st.markdown("**Company Registry Information** *(used for auto-generated description)*")
col1, col2, col3, col4 = st.columns(4)

with col1:
    st.session_state.form_data['foundation_date'] = st.text_input("📅 Foundation Date", value=st.session_state.form_data.get('foundation_date', ''))
with col2:
    st.session_state.form_data['location'] = st.text_input("📍 Location (City)", value=st.session_state.form_data.get('location', ''))
with col3:
    ...
    if activity_history:
        ...
        try:
            default_idx = activity_history.index(current_activity) + 1
        except ValueError:
            default_idx = 0

        selected = st.selectbox("🏭 Main Activity", options=options, index=default_idx, key="activity_select")

        if selected == NEW_SENTINEL or selected is None:
            new_activity = st.text_input("Type new activity", value=current_activity if current_activity not in activity_history else "", key="activity_new_input")
            st.session_state.form_data['main_activity'] = new_activity
        else:
            st.session_state.form_data['main_activity'] = selected
    else:
        st.session_state.form_data['main_activity'] = st.text_input("🏭 Main Activity", value=current_activity, key="activity_plain")

with col4:
    st.session_state.form_data['current_status'] = st.text_input("✅ Current Status", value=st.session_state.form_data.get('current_status', ''))

st.markdown("**Or paste description manually:**")
st.session_state.form_data['descricao'] = st.text_area("📄 Company Description/About *(auto-generated if empty)*", value=st.session_state.form_data['descricao'], height=80)

st.markdown("### 📞 Contact Information")
col1, col2, col3 = st.columns(3)
with col1:
    st.session_state.form_data['telefone'] = st.text_input("☎️ Main Phone", value=st.session_state.form_data['telefone'])
with col2:
    st.session_state.form_data['email'] = st.text_input("📧 Main Email", value=st.session_state.form_data['email'])
with col3:
    st.session_state.form_data['endereco'] = st.text_input("🏢 Address", value=st.session_state.form_data['endereco'])

st.markdown("**Additional Contacts** *(separated by comma)*")
col1, col2 = st.columns(2)
with col1:
    st.session_state.form_data['additional_phones'] = st.text_input("Additional Phones", value=st.session_state.form_data.get('additional_phones', ''))
with col2:
    st.session_state.form_data['additional_emails'] = st.text_input("Additional Emails", value=st.session_state.form_data.get('additional_emails', ''))

st.markdown("### 👥 Key Personnel")
col1, col2 = st.columns(2)
with col1:
    st.session_state.form_data['partner_name'] = st.text_input("Partner Name (Sócio-Administrador)", value=st.session_state.form_data['partner_name'])
with col2:
    st.session_state.form_data['partner_email'] = st.text_input("Partner Email", value=st.session_state.form_data['partner_email'])

st.markdown("### 📱 Social Media")
col1, col2, col3 = st.columns(3)
with col1:
    st.session_state.form_data['facebook'] = st.text_input("📘 Facebook", value=st.session_state.form_data['facebook'])
with col2:
    st.session_state.form_data['instagram'] = st.text_input("📷 Instagram", value=st.session_state.form_data['instagram'])
with col3:
    st.session_state.form_data['linkedin'] = st.text_input("💼 LinkedIn", value=st.session_state.form_data['linkedin'])

st.markdown("### 💡 Observations & Red Flags")
st.session_state.form_data['observations'] = st.text_area("Add your insights, red flags, and observations here", value=st.session_state.form_data['observations'], height=120)
